...
summarize_matrix <- function(x, na.rm=FALSE) {
    ...
    num_lt_0 <- apply(less_than_zero(x),1,sum,na.rm=na.rm)
    num_btw_1_and_5 <- apply(apply(x,2,is_between,a=1,b=5),1,sum,na.rm=na.rm)
    if (na.rm == TRUE) {
        num_na <- numeric(nrow(x))
    } else {
        num_na <- apply(is.na(x),1,sum)
    }
    df <- data.frame(mean,stdev,median,min,max,num_lt_0,num_btw_1_and_5,num_na)
    colnames(df) <- c("mean","stdev","median","min","max","num_lt_0","num_btw_1_and_5","num_na")
    return(df)
}

#setNames(data.frame(matrix(c(apply(x,1,mean),apply(x,1,sd),apply(x,1,median),apply(x,1,min),apply(x,1,max),apply(less_than_zero(x),1,sum),apply(apply(x,2,is_between,a=1,b=5),1,sum),apply(is.na(x),1,sum)),nrow=nrow(x),ncol=8)), c("mean","stdev","median","min","max","num_lt_0","num_btw_1_and_5","num_na"))
